...
tax <- read_csv('~/benefits-cliff/Forsyth_County_2022/tax_liability/tax_output.csv') %>%
...
benefits <- read_rds("~/benefits-cliff/Forsyth_County_2022/plots/data/benefits.rds") %>%
...
write_rds(difference, "~/benefits-cliff/Forsyth_County_2022/tax_liability/income_diff.rds")

# write out condensed json dataset for plotting with java script
difference %>%
  select(-hourly) %>%
  filter(pretax_inc <= 7500) %>%
  write_json("~/benefits-cliff/Forsyth_County_2022/plots/data/income_diff.json")
