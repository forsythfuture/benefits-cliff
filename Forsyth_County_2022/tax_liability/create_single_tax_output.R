...
tax <- read_csv('Forsyth_County_2022/tax_liability/tax_output.csv') %>%
...
benefits <- read_rds("Forsyth_County_2022/plots/data/benefits.rds") %>%
...
write_rds(difference, "Forsyth_County_2022/tax_liability/income_diff.rds")

# write out condensed json dataset for plotting with java script
difference %>%
  select(-hourly) %>%
  filter(pretax_inc <= 7500) %>%
  write_json("Forsyth_County_2022/plots/data/income_diff.json")
