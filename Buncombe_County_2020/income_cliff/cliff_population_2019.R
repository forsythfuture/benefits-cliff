...
# 2019 Household size Buncombe County counts by foodstamp status
...
income_2019_HH_foodstamps <- test_2019 %>%
  ...
  mutate(total = size * HHWT) %>%
  ungroup() %>%
  select(size, HHINCOME, HHWT, total, FOODSTMP)

#totals by household size by foodstamp status
totals_by_household_size_foodstamps <- income_2019_HH_foodstamps %>%
  group_by(size, FOODSTMP) %>%
  summarise(sum = sum(total))
...
# 2019 race/ethnicity Buncombe County counts by foodstamp status
...
income_2019_race_foodstamps <- test_2019 %>%
  ...
  select(SERIAL, size, HHINCOME, HHWT, race_ethnicity, FOODSTMP) %>%
  #make a new HHWT variable that divides the HHWT by the size of the household to get the individual weight
  mutate(newHHWT = if_else(SERIAL %in% multiple_serials, HHWT / size, HHWT)) %>%
  group_by(size, HHINCOME, race_ethnicity, FOODSTMP) %>%
  # now group by size and income and race
  # we're grouping by income because we will sum household weights by income and race
  # so they are aggregated and we don't have multiple rows of the same income
  # sum newHHWT
  ...
  mutate(total = size * officialHHWT) %>%
  ungroup() %>%
  select(size, race_ethnicity, HHINCOME, officialHHWT, total, FOODSTMP)

#totals by race/ethnicity by foodstamp status
totals_by_race_ethnicity_foodstamps <- income_2019_race_foodstamps %>%
  group_by(race_ethnicity, FOODSTMP) %>%
  summarise(sum = sum(total))
